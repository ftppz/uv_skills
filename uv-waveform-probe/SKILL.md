...
- The signal you want to observe is **visible** at the fe stage (top-level port, or an internal RTL signal inside a non-blackbox module).
  - Signals **inside a blackbox IP (DCP) cannot be probed** with this flow — only its top-level ports or the nets connecting it to the top.
...
- A blackbox internal clock is **not allowed** (the probe_net docs state it explicitly: `Specifying a blackbox internal clock as the sampling clock is not supported`).
